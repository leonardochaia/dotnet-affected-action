...
// The input default is what keeps a newer major from ever being installed. The guard
...
describe('deprecation notice', () => {
  test('says the version is deprecated', () => {
    expect(DEPRECATION_NOTICE).toMatch(/deprecated/)
  })

  // The whole point of the notice: somewhere to go.
  test('names the action version to move to', () => {
    expect(DEPRECATION_NOTICE).toMatch(/@v7/)
  })
})
